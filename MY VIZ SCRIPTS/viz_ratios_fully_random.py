...
def calculate_ratios(returns, risk_free_rate=0.02):
    ...
    sharpe = np.mean(excess_returns) / \
        np.std(returns) if np.std(returns) != 0 else 0
    downside_returns = np.minimum(excess_returns, 0)
    sortino = np.mean(excess_returns) / \
        np.std(downside_returns) if np.std(downside_returns) != 0 else np.inf
    ...
    calmar = np.mean(excess_returns) / \
        max_drawdown if max_drawdown != 0 else np.inf

    return sharpe, sortino, omega, calmar


def generate_balance(days, seed=None):
    if seed is not None:
        np.random.seed(seed)
...
def custom_score(final_total, sortino, omega, k=1000):
    total_return = (final_total - 1000) / 1000 * \
        100  # Assuming initial balance is 1000
    smooth_sortino_returns = smooth_sortino_metric(sortino, total_return, k)
    return smooth_sortino_returns * (omega ** 2)


def run_analysis(num_charts, days, num_charts_to_show):
    balances = []
    all_ratios = []
    final_totals = []
    custom_scores = []
    ...
    for i in range(num_charts):
        balance = generate_balance(days, seed=i, )
    ...
    sort_criteria = ['Sharpe', 'Sortino', 'Omega', 'Calmar', 'Final Total',]
    # sort_criteria = ['Sortino', 'Custom Score']

    for i, criterion in enumerate(sort_criteria):
        if criterion == 'Final Total':
            sorted_indices = sorted(
                range(num_charts), key=lambda k: final_totals[k], reverse=True)
        elif criterion == 'Custom Score':
            sorted_indices = sorted(
                range(num_charts), key=lambda k: custom_scores[k], reverse=True)
        else:
            sorted_indices = sorted(
                range(num_charts), key=lambda k: all_ratios[k][i], reverse=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate and analyze portfolio balance charts.")
    parser.add_argument("--num_charts_to_gen", type=int,
                        default=40, help="Number of charts to generate")
    parser.add_argument("--days", type=int, default=60,
                        help="Number of days for each chart")
    parser.add_argument("--num_charts_to_show", type=int,
                        default=10, help="Number of top charts to display")
    args = parser.parse_args()

    run_analysis(args.num_charts_to_gen, args.days, args.num_charts_to_show)
