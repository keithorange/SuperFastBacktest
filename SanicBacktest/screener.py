from asset_names import popular_assets, cryptocurrencies, us_market_stocks
...
strategy_files = [f for f in os.listdir(
    strategy_path) if f.endswith('.py') and not f.startswith('__')]
...
class StockScreener:
    ...
    def download_data(self):
        ...
        for symbol in self.symbols:
            ...
            stock_data = download_data(
                symbol, num_candles=self.num_candles, interval=self.interval, redownload=True)
    ...
    def dollar_volume_filter(self, min_dollar_volume):
        ...
        for symbol, stock_data in self.data.items():
            stock_data['DollarVolume'] = stock_data['Volume'] * \
                stock_data['Close']
    ...
    def volatility_filter(self, lookback_days, min_volatility, sort_direction='desc'):
        ...
        for symbol, stock_data in self.data.items():
            ...
            rolling_volatility = stock_data['Return'].rolling(
                lookback_days).std().dropna()
            avg_volatility = rolling_volatility.mean()

            if avg_volatility >= min_volatility:
                filtered_symbols.append(symbol)
        if sort_direction == 'desc':
            filtered_symbols.sort(key=lambda x: self.data[x]['Return'].rolling(
                lookback_days).std().mean(), reverse=True)
        else:
            filtered_symbols.sort(
                key=lambda x: self.data[x]['Return'].rolling(lookback_days).std().mean())
        return filtered_symbols

    def strategy_filter(self, strategy_name, n_periods=1, signal_type='entry'):
        filtered_symbols = []
        strategy_func_or_class = self.load_strategy_func_or_class(
            strategy_name)
        for symbol, stock_data in self.data.items():
            if callable(strategy_func_or_class):
                ...
                # Assuming strategies have a 'run' method returning entries and exits
    ...
    def load_strategy_func_or_class(self, strategy_name):
        ...
        strategy_func_or_class_name = ''.join(
            [part.capitalize() for part in strategy_name.split('_')])
        ...
        for filter_func, filter_args in filters:
            ...
            if self.verbose:
                print(f"({count+1}) Filter {filter_func.__name__} with args {filter_args}:\n    input symbols={
                      input_symbol_count}, output symbols={len(filtered_symbols)}\n")
            count += 1
        return filtered_symbols
...
class StockPlotter:
    ...
    def calculate_heikin_ashi(self, data):
        ha_data = data.copy()
        ha_data['Close'] = (data['Open'] + data['High'] +
...
if __name__ == "__main__":
    ...
    filters = [
        ...
        (screener.volatility_filter, {
         'lookback_days': 10, 'min_volatility': 0.001, 'sort_direction': 'desc'}),
        ...
        (screener.strategy_filter, {
         'strategy_name': 'ut_bot_strategy', 'n_periods': 2, 'signal_type': 'entry'})
    ]
