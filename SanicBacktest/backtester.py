...
def backtest_strategy(
    ...
    init_cash: float,
    interval: str,
    leverage: float = 40, #10,
    entry_signal_stacking: bool = False,
    max_concurrent_trades: int = 1,
    use_fixed_size: bool = True,
    trade_size: float = 0.1,
    fees: float = 0.0005, # 2x per trade (happens 1x per buy, 1x per sell) costs (fees + slippage + spread etc.)
    ...
    """
    WAIT_AFTER_EXIT_N_CANDLES = 2
    WAIT_AFTER_ENTRY_N_CANDLES = 2
   ...
   # Near the top of the function, after other initializations
    ...
    for i in range(len(close_prices_raw)):
        if len(portfolio.long_open_trades) or len(portfolio.short_open_trades):
            # put debugger here to read stuff
            pass
        ...
        if i > 0:
            ...
            if (long_exits[i-1 if exit_on_next_candle else i] or
                short_exits[i-1 if exit_on_next_candle else i] or
                is_end_of_day_exit_time or 
                is_random_exit or
                is_in_exit_time_range):
                # (include_uncompleted and i == len(close_prices_raw) - 1) handled above nautrally):
                
                is_in_long = long_exits[i-1 if exit_on_next_candle else i]==1
        ...
        allow_entry_short = (
            is_trade_allowed and
            short_entries[i] and  # Check for short entry condition
        ...
        is_long_entry = long_entries[i] == 1
...
def run_backtesting_for_symbol(symbol, raw_data, normalized_data, plot, interval, strategy_func, strategy_params, **kwargs):
    ...
    # extract leverage from strategy params to pass into backtesting_fn itself
    strategy_params_copy = strategy_params.copy()
    leverage = strategy_params_copy.pop('leverage')

    # Use pre-normalized data for signal generation
    long_entries, long_exits, short_entries, short_exits  = strategy_func(normalized_data, **strategy_params_copy)
    ...
    if plot: 
        plot_backtest_results(result, symbol, interval, leverage)
...
def plot_backtest_results(result, symbol, interval, leverage, save_to_file=True):

    if save_to_file:
        plot_dir = 'backtest_plots'
        os.makedirs(plot_dir, exist_ok=True)
        base_filename = f"{symbol}_{interval}_{leverage}"
        

    def check_if_new_best_in_dir() -> bool:
        # NOTE: ONLY SAVE THE BEST IMAGE (WILL BE FASTER!)
        # Fast check for the best metric value
        old_best_metric = float('-inf')
            ...
            if filename.startswith(base_filename) and filename.endswith('_backtest_results.png'):
                ...
                if match:
                    try:
                        ...
                        old_best_metric = max(old_best_metric, file_metric)
                    except ValueError:
        ...
        # return if BETTER THAN EXISTING BEST
        return result['metrics']['total_return'] >= old_best_metric

    # # ONLY conitnue if BEST
    # if not check_if_new_best_in_dir():
    #     return         
    

    # # PLOT ONLY POSITIVE RESULTS
    ...
    try:
        ...
        ax1.set_title(f"{symbol} {leverage} {interval} backtest")
        ax1.legend()
        ...
        # Add overall title
        plt.suptitle(f"{symbol} {leverage}x {interval} -> %{metrics["total_return"]})", fontsize=20, y=0.995)
        ...
        if save_to_file:
            ...
            filename = f"{symbol}_{interval}_{leverage}_%={result['metrics']['total_return']}_backtest_results.png"
            filepath = os.path.join(plot_dir, filename)
            fig.savefig(filepath, dpi=100)


            # # ONLY conitnue if BEST
            # if check_if_new_best_in_dir():
            #     # TODO buggy !
            ...
            metrics_filename = f"{symbol}_{interval}_{leverage}_%={result['metrics']['total_return']}_metrics.txt"
